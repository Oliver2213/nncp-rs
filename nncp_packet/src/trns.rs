...
impl TrnsPacket {
    /// Create a new transit packet with the given path
    ...
    /// Encode the transit packet with the given niceness level
    ...
    /// Decode a transit packet from a reader
...
mod tests {
    ...
    fn test_trns_packet_roundtrip() {
        let path = "node1/node2/node3";
        let trns_packet = TrnsPacket::new(path).unwrap();
